...
<div class="min-h-screen bg-slate-950 p-8 text-white">
    <div class="mx-auto max-w-2xl">
        ...
        @if ($saved)
            <p class="mb-4 rounded-lg border border-emerald-800 bg-emerald-950/30 p-3 text-sm text-emerald-400">{{ $saved }}</p>
        @endif

        <form wire:submit="save" class="space-y-6">
            <div class="rounded-xl border border-slate-800 bg-slate-900 p-6">
                <h2 class="mb-4 text-sm font-semibold text-slate-300">Datos del negocio</h2>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label class="mb-1 block text-sm text-slate-300">Nombre comercial</label>
                    ...
                    <div>
                        <label class="mb-1 block text-sm text-slate-300">Razón social</label>
                        <input type="text" wire:model="legal_name" class="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-white focus:border-indigo-500 focus:outline-none">
                    </div>
                    <div>
            ...
            <div class="rounded-xl border border-slate-800 bg-slate-900 p-6">
                <h2 class="mb-1 text-sm font-semibold text-slate-300">Política de inventario negativo</h2>
                <p class="mb-4 text-xs text-slate-500">Qué hacer cuando una venta o ajuste dejaría la existencia de un insumo por debajo de cero.</p>
                <select wire:model="inventario_negativo" class="w-full max-w-sm rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-white">
            ...
            <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium hover:bg-indigo-500">Guardar configuración</button>
        </form>
    </div>
</div>
